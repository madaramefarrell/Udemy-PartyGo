...
import bson
from party.forms import BasicPartyorm, EditPartyForm, CancelPartyForm
    ...
    if party and party.host == user.id:
        ...
        if request.method == 'POST' and form.validate():
            ...
            if not error:
                ...
                image_url = upload_image_file(request.files.get('photo'), 'party_go', str(party.id))
                if image_url:
...
def manage(party_page_number=1):
    user = User.objects.filter(email=session.get('email')).first()
    if user:
        paries = Party.objects.filter(host=user.id).order_by('-start_datetime').paginate(page=party_page_number,
                                                                                         per_page=4)
        return render_template('party/manage.html', parties=paries)
    else:
        abort(404)
...
def explore(party_page_number=1):
    ...
    try:
        ...
        parties = Party.objects(location__near=[lng, lat], location__max_distance=100000,
